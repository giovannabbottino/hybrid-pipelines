...
Após o recebimento da resposta do LLM, o conteúdo é interpretado pelo serviço e as menções identificadas são realinhadas às respectivas ocorrências no texto original. Esse procedimento permite associar cada entidade ou conceito ao trecho efetivamente encontrado no documento.

O conjunto de menções pode ser complementado por regras heurísticas aplicadas ao próprio texto, destinadas à recuperação de padrões lexicais ou conceitos que não tenham sido identificados inicialmente pelo modelo.
...
Cada menção obtida na etapa anterior é utilizada como termo de busca para recuperar possíveis candidatos na Wikidata. Os recursos encontrados são posteriormente comparados para a seleção daquele que melhor representa a entidade mencionada no texto.
...
A terceira etapa compreende a construção, a validação e, quando necessário, a correção da representação em RDF/Turtle.
...
Quando a representação é considerada válida, o fluxo segue para a etapa de salvamento. Caso contrário, são aplicados procedimentos de correção, entre os quais estão a normalização de aspas, a inclusão de ponto final em declarações incompletas e a remoção de trechos que impeçam a interpretação do RDF.

A implementação também dispõe de uma estratégia determinística para a construção do grafo a partir das entidades e relações previamente obtidas. Essa estratégia pode ser utilizada como alternativa quando a representação produzida pelo modelo não pode ser validada.

Caso ainda existam tentativas disponíveis, uma nova chamada ao LLM também pode ser realizada, utilizando o conteúdo inválido e as informações referentes ao erro encontrado durante a análise. O número de tentativas é limitado, evitando ciclos indefinidos de correção. Caso nenhuma alternativa produza uma representação válida, o processamento é interrompido.
...
Conforme apresentado na Figura~\ref{fig:experimento-c}, tanto o RDF considerado válido na primeira análise quanto aquele submetido aos procedimentos de correção convergem para essa etapa.
...
No perfil utilizado, até 16 menções são encaminhadas à etapa de resolução das entidades. As chamadas realizadas ao Ollama utilizam semente \texttt{42}, temperatura \texttt{0} e limite de \texttt{1536} \textit{tokens} de saída. Quando os parâmetros da requisição não são modificados, o LLM dispõe inicialmente de uma tentativa para gerar o RDF, enquanto os mecanismos locais de reparo e a estratégia determinística permanecem disponíveis como alternativas em caso de falha na validação.
...
Os mecanismos locais de reparo e de construção determinística também podem influenciar o caminho utilizado para a obtenção da representação final. Dessa forma, representações produzidas diretamente pelo LLM, corrigidas após a geração ou construídas pela estratégia determinística correspondem a diferentes caminhos possíveis dentro do mesmo fluxo experimental.
...
Em síntese, o experimento com \textit{hybrid pipelines} combina o modelo de linguagem, informações provenientes da Wikidata e procedimentos locais de processamento e validação. O LLM é utilizado na extração das menções e na geração inicial da representação RDF/Turtle; a Wikidata fornece candidatos, identificadores e relações estruturadas; e as rotinas locais realizam o processamento, a validação e, quando necessário, a correção do resultado. O grafo somente é encaminhado à etapa final após a obtenção de uma representação RDF/Turtle considerada válida.
